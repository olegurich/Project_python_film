...
def get_search_keyword() -> str: 
    ...
    """
    keyword = input("Введите ключевое слово для поиска в названии фильма и нажмите 'Enter': ").strip()
    if not keyword:
...
def ask_for_next_page() -> bool:
    ...
    """
    response = input("Показать следующие 10 фильмов? (Enter — да, '0' и 'Enter' — выход): ").strip().lower()
    return response != "0"
...
def get_genre_id(cursor) -> int|None:
    ...
    genre_id_raw = input("Введите номер жанра и нажмите 'Enter': ").strip()
